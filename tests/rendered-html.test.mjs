...
test("keeps all You.com credentials server-side and implements the three-stage pipeline", async () => {
  ...
  assert.match(page, /Download content update/);
  assert.match(page, /data:text\/markdown;charset=utf-8/);
  assert.match(page, /alexandria-content-update-/);
  assert.match(page, /Publishing still requires an authenticated CMS connection/);
